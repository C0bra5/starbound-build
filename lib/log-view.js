...
export default class LogView extends View {
		...
		this.div({ tabIndex: -1, class: 'build tool-panel native-key-bindings' }, () => {
			this.div({ class: 'heading', outlet: 'panelHeading' }, () => {
				this.div({ class: 'control-container opaque-hover' }, () => {
					this.button({ class: 'btn btn-default icon icon-zap', click: 'Start', title: 'Start starbound' });
					this.button({ class: 'btn btn-default icon icon-trashcan', click: 'clearOutput', title: 'Clear log' });
					this.button({ class: 'btn btn-default icon icon-x', click: 'close', title: 'close log' });
	...
	constructor(...args) {
		...
		this.terminal = new Terminal({
			...
			scrollback: atom.config.get('starbound-build.terminalScrollback')
		});
		...
		atom.config.observe('starbound-build.panelVisibility', ::this.visibleFromConfig);
		atom.config.observe('starbound-build.panelOrientation', ::this.orientationFromConfig);
		atom.config.observe('starbound-build.hidePanelHeading', (hide) => {
			hide && this.panelHeading.hide() || this.panelHeading.show();
		});
		atom.config.observe('starbound-build.overrideThemeColors', (override) => {
		...
		atom.commands.add('atom-workspace', 'starbound-build:toggle-panel', ::this.toggle);
	}
	...
	resizeMoved(ev) {
		...
		switch (atom.config.get('starbound-build.panelOrientation')) {
			case 'Bottom': {
	...
	resizeToNearestRow() {
		if (-1 !== [ 'Top', 'Bottom' ].indexOf(atom.config.get('starbound-build.panelOrientation'))) {
	...
	attach(force = false) {
		if (!force) {
			switch (atom.config.get('starbound-build.panelVisibility')) {
				case 'Hidden':
				case 'Show on Error':
					return;
			}
		...
		const orientation = atom.config.get('starbound-build.panelOrientation') || 'Bottom';
	...
	detach(force) {
		...
		if (this.panel && (force || 'Keep Visible' !== atom.config.get('starbound-build.panelVisibility'))) {
			this.panel.destroy();
			this.panel = null;
		}
	}
	...
	reset() {
		...
		this.buildTimer.text(LogView.initialTimerText());
		this.titleTimer = 0;
		this.terminal.reset();
	...
	start() {
		atom.commands.dispatch(atom.views.getView(atom.workspace), 'starbound-build:start-game');
	}
	...
	gameStarted() {
		...
		this.reset();
		this.attach();
		if (atom.config.get('starbound-build.stealFocus')) {
			this.focus();
		}
		this.updateTitle();
	}
	...
	gameClosed(success) {
		if (!success && !this.isAttached()) {
			this.attach(atom.config.get('starbound-build.panelVisibility') === 'Show on Error');
		}
