...
class TargetManager extends EventEmitter {
	constructor() {
		...
		atom.commands.add('atom-workspace', 'starbound-build:refresh-targets', () => this.refreshTargets());
		atom.commands.add('atom-workspace', 'starbound-build:select-active-target', () => this.selectActiveTarget());
	}
	...
	refreshTargets(refreshPaths) {
			...
			return Promise.all(settingsPromise).then((settings) => {
				...
				settings.forEach((setting, index) => {
					if (setting.keymap && !setting.atomCommandName) {
						setting.atomCommandName = `starbound-build:trigger:${setting.name}`;
					}
		...
		return Promise.all(pathPromises).then(pathTargets => {
			...
			if (atom.config.get('starbound-build.notificationOnRefresh')) {
	...
	selectActiveTarget() {
		if (atom.config.get('starbound-build.refreshOnShowTargetList')) {
			this.refreshTargets();
		}
