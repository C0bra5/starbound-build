...
export default class StatusBarView extends View {
  constructor(statusBar, ...args) {
    ...
    atom.config.observe('starbound-build.statusBar', () => this.attach());
    atom.config.observe('starbound-build.statusBarPriority', () => this.attach());
  }

  attach() {
    this.destroy();

    const orientation = atom.config.get('starbound-build.statusBar');
    ...
    this.statusBarTile = this.statusBar[`add${orientation}Tile`]({
      item: this,
      priority: atom.config.get('starbound-build.statusBarPriority')
    });
